...
#define CUST_EINT_TOUCH_PANEL_TYPE EINTF_TRIGGER_RISING
...
char data[128] = {0};
	...
	for(i = 0; i < point_num; i++)  
	{
        ...
        //add for rotation  // wanghe 2013-06-01  wanghe 2013-08-06 for factory 180
#if 0
        //	 if((0 == strncmp(MTK_LCM_PHYSICAL_ROTATION, "180", 3))&&!(FACTORY_BOOT == get_boot_mode()|| RECOVERY_BOOT == get_boot_mode()) && (cinfo->y[i] < TPD_RES_Y))
		if((0 == strncmp(MTK_LCM_PHYSICAL_ROTATION, "180", 3))/*&&!(FACTORY_BOOT == get_boot_mode()|| RECOVERY_BOOT == get_boot_mode()) */&& (cinfo->y[i] < TPD_RES_Y))
		{
...
static void check_gesture(int gesture_id)
{
    ...
    printk("\n<xieen>FTS_GESTURE ----- kaka gesture_id==%d\n ",gesture_id);

	switch(gesture_id)
	{
		case GESTURE_LEFT:
		      input_report_key(tpd->dev, KEY_LEFT, 1); //KEY_LEFT, 1);
			    input_sync(tpd->dev);
			     input_report_key(tpd->dev, KEY_LEFT, 0); //KEY_LEFT, 0);
			    input_sync(tpd->dev);
                         //kpd_touchpanel_gesture_handler(KEY_TPGESTURE_LEFT);
			break;
		case GESTURE_RIGHT:
		       input_report_key(tpd->dev, KEY_RIGHT, 1); //KEY_RIGHT, 1);
			    input_sync(tpd->dev);
			     input_report_key(tpd->dev, KEY_RIGHT, 0); //KEY_RIGHT, 0);
			    input_sync(tpd->dev);
                         //kpd_touchpanel_gesture_handler(KEY_TPGESTURE_RIGHT);
			break;
		case GESTURE_UP:
			input_report_key(tpd->dev, KEY_UP, 1); //KEY_UP, 1);
			    input_sync(tpd->dev);
			     input_report_key(tpd->dev, KEY_UP, 0); //KEY_UP, 0);
			    input_sync(tpd->dev);
                         //kpd_touchpanel_gesture_handler(KEY_TPGESTURE_UP);    
			break;
		case GESTURE_DOWN:
			input_report_key(tpd->dev, KEY_DOWN, 1); //KEY_DOWN, 1);
			    input_sync(tpd->dev);
			     input_report_key(tpd->dev, KEY_DOWN, 0); //KEY_DOWN, 0);
			    input_sync(tpd->dev);
                         //kpd_touchpanel_gesture_handler(KEY_TPGESTURE_DOWN);    
			break;
		case GESTURE_DOUBLECLICK:
			/*input_report_key(tpd->dev, KEY_TPGESTURE_DOUBLE, 1); //KEY_U, 1);
			    input_sync(tpd->dev);
			     input_report_key(tpd->dev, KEY_TPGESTURE_DOUBLE, 0); //KEY_U, 0);
			    input_sync(tpd->dev);*/
                         //kpd_touchpanel_gesture_handler(KEY_TPGESTURE_DOUBLE);    
			break;
		case GESTURE_O:
			input_report_key(tpd->dev, KEY_O, 1); //KEY_DOWN, 1);
			    input_sync(tpd->dev);
			     input_report_key(tpd->dev, KEY_O, 0); //KEY_DOWN, 0);
			    input_sync(tpd->dev);
 			/*input_report_key(tpd->dev, KEY_O, 1);
			    input_sync(tpd->dev);
			     input_report_key(tpd->dev, KEY_O, 0);
			    input_sync(tpd->dev);*/
			    //kpd_touchpanel_gesture_handler(KEY_TPGESTURE_O); 
			break;
		case GESTURE_W:
			input_report_key(tpd->dev, KEY_W, 1);
			    ...
			    input_sync(tpd->dev);
				//kpd_touchpanel_gesture_handler(KEY_TPGESTURE_W); 			    
			break;
		case GESTURE_M:
			input_report_key(tpd->dev, KEY_M, 1);
			    ...
			    input_sync(tpd->dev);
				//kpd_touchpanel_gesture_handler(KEY_TPGESTURE_M); 			    
			break;
		case GESTURE_E:
			input_report_key(tpd->dev, KEY_E, 1);
			    ...
			    input_sync(tpd->dev);
			    //kpd_touchpanel_gesture_handler(KEY_TPGESTURE_E); 
			break;
		case GESTURE_C:
			input_report_key(tpd->dev, KEY_C, 1);
			 ...
			 input_sync(tpd->dev);
			 	//kpd_touchpanel_gesture_handler(KEY_TPGESTURE_C); 
			break;
		...
		case GESTURE_S:
		 ...
		 input_sync(tpd->dev);
		 //kpd_touchpanel_gesture_handler(KEY_TPGESTURE_S); 
		break;
		...
		case GESTURE_V:
		 ...
		 input_sync(tpd->dev);
		 //kpd_touchpanel_gesture_handler(KEY_TPGESTURE_V); 
		break;
		...
		case GESTURE_Z:
		 ...
		 input_sync(tpd->dev);
		 //kpd_touchpanel_gesture_handler(KEY_TPGESTURE_Z); 
			break;
		default:
...
static int ft5x0x_read_Touchdata(void)
{
    ...
    printk("<xieen>FTS_GESTURE buff[0]=0x%x, buff[1]=0x%x\n", buf[0], buf[1]);
    /* FW ?¡À?¨®??3?¨º?¨º? */
    if (0x24 == buf[0])// || 0x23 == buf[0] || 0x22 == buf[0] || 0x20 == buf[0] || 0x21 == buf[0])
    {
        gestrue_id = 0x24;
 ...
 { 
   ...
   u8 state;
	...
	do
	{
		//mt65xx_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);  //modified by xen for test 20140429
		...
		printk("<xieen> FTS_GESTURE state=%d\n", state);
		if(state ==1)
		{
...
static void tpd_eint_interrupt_handler(void)
{
	...
	TPD_DEBUG_PRINT_INT;
	printk("<xieen>focaltek interrupt handler\n");
	tpd_flag = 1;
...
#if 0  //modified by xen for test 20140429
...
static int tpd_local_init(void)
{
 ...
 static void tpd_suspend( struct early_suspend *h )
 {
	...
	if (tpd_proximity_flag == 1)
	{
        ...
        fts_write_reg(i2c_client, 0xd1, 0x3f);   // default is 0x1F(only double-click can be detected, 0x3F can also detect letters)
	...
	// mt65xx_eint_mask(CUST_EINT_TOUCH_PANEL_NUM); //modified by xen for test 20140429
...
MODULE_DESCRIPTION("YongYiDa tp  Driver");

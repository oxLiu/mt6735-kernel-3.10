...
{
#ifdef MTK_MSDC_BRINGUP_DEBUG
	if(apmixed_reg_base1 && topckgen_reg_base && clk_pericfg_base){ 
		...
		pr_err(" PERI_PDN_STA0   [0x%p][bit13=msdc0, bit14=msdc1,0:on,1:off]=0x%x\n",
			(clk_pericfg_base + MSDC_PERI_PDN_STA0_OFFSET), sdr_read32(clk_pericfg_base + MSDC_PERI_PDN_STA0_OFFSET));
	}else
		pr_err(" apmixed_reg_base1 = %p, topckgen_reg_base = %p,clk_pericfg_base = %p\n", 
			apmixed_reg_base1,topckgen_reg_base,clk_pericfg_base);
#endif
}
	...
	if (intsts & cmdsts) {
		...
		if ((intsts & MSDC_INT_CMDRDY) || (intsts & MSDC_INT_ACMDRDY) ||
			...
			if((cmd->opcode == 5) && emmc_do_sleep_awake) {
				pr_err("XXX CMD<5>CRC not reset hw...\n");
			} else {
				msdc_reset_hw(host->id);
			}
			...
			if ((cmd->opcode != 52) && (cmd->opcode != 8) && (cmd->opcode != 5)
				...
				msdc_dump_info(host->id);
			if((cmd->opcode == 5) && emmc_do_sleep_awake) {
				pr_err("XXX CMD<5> tmo not reset hw...\n");
				msdc_dump_info(host->id);
			} else {
				msdc_reset_hw(host->id);
			}
...
#ifdef CFG_DEV_MSDC2 //FIXME: For 6630
    //FIXME@CCJ mt_vcore_dvfs_disable_by_sdio(1, true);
#endif
...
#ifdef CFG_DEV_MSDC2 //FIXME: For 6630
    //FIXME@CCJ mt_vcore_dvfs_disable_by_sdio(1, false);
#endif    
...
//	msdc_select_clksrc(host, hw->clk_src); // no need select clock cause 400M can div to 260K
    ...
    host->saved_para.pad_tune0 = (((hw->cmdrrddly & 0x1F) << 22) | ((hw->cmdrddly & 0x1F) << 16) | ((hw->datwrddly & 0x1F) << 0));//sdr_read32(MSDC_PAD_TUNE0);

...
void do_ptim(void)
{
	...
	PMICLOG("[do_ptim] end %d %d ",pmic_get_register_value(PMIC_RG_AUXADC_SMPS_CK_PDN),pmic_get_register_value(PMIC_RG_AUXADC_SMPS_CK_PDN_HWEN));

	//PMICLOG("[do_ptim] volt_1=%d, curr_1=%d\n", ptim_bat_vol, ptim_R_curr);
}


kal_uint32 ptim_cnt=0;

void ptim_int_handler(void)
{
	kal_uint32 vbat_reg;

	ptim_cnt++;
	PMICLOG("[do_ptim] 0xee8=0x%x  0x2c6=0x%x  ptim_cnt=%d\n", upmu_get_reg_value(0xee8),upmu_get_reg_value(0x2c6),ptim_cnt);
	...
	ptim_bat_vol=(vbat_reg*3*1800)/32768;
...
int get_rac_val(void)
{
	...
	{        
		...
		PMICLOG("[ZF1] [0xe3a]=0x%x [0xe3c]=0x%x\n"
				,upmu_get_reg_value(0xe3a),upmu_get_reg_value(0xe3c)
				);
		
		while(pmic_ptim_rdy!=1)
		{
			msleep(20);
			PMICLOG("ptim timeout [0xe94]=0x%x [0x23c]=0x%x [0x242]=0x%x [0x24e]=0x%x [0x25a]=0x%x [0x272]=0x%x\n"
				,upmu_get_reg_value(0xe94),upmu_get_reg_value(0x23c)
				,upmu_get_reg_value(0x242),upmu_get_reg_value(0x24e)
				,upmu_get_reg_value(0x25a),upmu_get_reg_value(0x272)
				);

		PMICLOG("[0x278]=0x%x [0x288]=0x%x [0x28a]=0x%x [0xee8]=0x%x [0x298]=0x%x [0x0eac]=0x%x\n"
			,upmu_get_reg_value(0x278),upmu_get_reg_value(0x288)
			,upmu_get_reg_value(0x28a),upmu_get_reg_value(0xee8)
			,upmu_get_reg_value(0x298),upmu_get_reg_value(0x0eac)
            );
		...
		PMICLOG("[1,Trigger ADC PTIM mode] volt1=%d, curr_1=%d\n", ptim_bat_vol, ptim_R_curr);
		volt_1=ptim_bat_vol;
		curr_1=ptim_R_curr;
	
        PMICLOG("[2,enable dummy load]");
        enable_dummy_load(1);
        msleep(50);
		//Wait --------------------------------------------------------------
		...
		PMICLOG("[ZF2] [0xe3a]=0x%x [0xe3c]=0x%x\n"
				,upmu_get_reg_value(0xe3a),upmu_get_reg_value(0xe3c)
				);
		...
		PMICLOG("[1,Trigger ADC PTIM mode] volt2=%d, curr_2=%d\n", ptim_bat_vol, ptim_R_curr);
		volt_2=ptim_bat_vol;
		curr_2=ptim_R_curr;
        ...
        //Calculate Rac------------------------------------------------------
        if( (curr_2-curr_1) >= 700 && (volt_1-volt_2)>=7 ) //40.0mA
        {
            rac_cal=((volt_1-volt_2)*10000)/(curr_2-curr_1); //m-ohm
        ...
        else if( (curr_1-curr_2) >= 700 && (volt_2-volt_1)>=7 ) //40.0mA
        {
            rac_cal=((volt_2-volt_1)*10000)/(curr_1-curr_2); //m-ohm
        ...
        PMICLOG("[6,Calculate Rac] %d,%d,%d,%d,%d,%d,%d\n",
            volt_1,volt_2,curr_1,curr_2,rac_cal,ret,retry_count);
...
int get_dlpt_imix(void)
{
	...
	{		 
		...
		PMICLOG("[ZF3] [0xe3a]=0x%x [0xe3c]=0x%x\n"
				,upmu_get_reg_value(0xe3a),upmu_get_reg_value(0xe3c)
				);
	...
	imix=curr_avg+(volt_avg-3250)*10000/rac_val_avg;

		PMICLOG("[dlpt_Imix] %d,%d,%d,%d,%d,%d,%d\n",volt_avg,curr_avg,3250,rac_val_avg,imix,BMT_status.SOC,bat_get_ui_percentage());
    ...
    do
    {
        ...
        {   
            ...
            else
            {
                ...
                g_imax_val=get_dlpt_imix();//get_dlpt_imax_avg();
...
#ifdef DLPT_FEATURE_SUPPORT
	pmic_register_interrupt_callback(30,ptim_int_handler);	
#endif //#ifdef DLPT_FEATURE_SUPPORT
...
void PMIC_INIT_SETTING_V1(void)
{
    ...
    //--------------------------------------------------------
        ...
        PMICLOG("[Kernel_PMIC_INIT_SETTING_V1] 2015-02-05...\n");

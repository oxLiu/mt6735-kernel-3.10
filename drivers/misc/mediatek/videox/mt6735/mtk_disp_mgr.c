...
#else
        DISPERR("mhl path: OVL needs to support time-sharing mechanism. \n");
        ret = -1;
#endif
    }
...
#ifdef CONFIG_ALL_IN_TRIGGER_STAGE
	captured_session_input[0].config_layer_num = DISP_HW_MAX_LAYER;
	for (i=0 ; i<DISP_HW_MAX_LAYER ; i++)
	{
		captured_session_input[0].config[i].layer_enable = 0;
	}
#endif

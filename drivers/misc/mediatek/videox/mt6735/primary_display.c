...
int primary_display_merge_session_cmd(disp_session_config *config)
{
	...
	session_input = &cached_session_input[DISP_SESSION_PRIMARY - 1];
	cached_session_input[DISP_SESSION_PRIMARY - 1] = captured_session_input[DISP_SESSION_PRIMARY - 1];
	cached_session_input[DISP_SESSION_PRIMARY - 1].session_id = config->session_id;

	if (output_type == DISP_OUTPUT_MEMORY || is_multipass_trigger)
	{
		cached_session_output[DISP_SESSION_PRIMARY - 1] = captured_session_output[DISP_SESSION_PRIMARY - 1];
	}
	...
	if (output_type == DISP_OUTPUT_MEMORY || pgc->session_mode == DISP_SESSION_DECOUPLE_MIRROR_MODE)
	{
		...
		cached_session_output[DISP_SESSION_PRIMARY - 1] = captured_session_output[DISP_SESSION_PRIMARY - 1];
		mutex_unlock(&session_config_mutex);
		primary_output = &cached_session_output[DISP_SESSION_PRIMARY - 1];
            ...
            */
            if (is_multipass_trigger)
            {
                _trigger_ovl_to_memory(pgc->ovl2mem_path_handle, pgc->cmdq_handle_ovl1to2_config,
                                        (fence_release_callback)_ovl_fence_release_callback, 5);
                is_multipass_trigger = 0;
            }
            else
            {
                _trigger_ovl_to_memory(pgc->ovl2mem_path_handle, pgc->cmdq_handle_ovl1to2_config,
                                        (fence_release_callback)_ovl_fence_release_callback, DISP_SESSION_DECOUPLE_MODE);
            }
        }

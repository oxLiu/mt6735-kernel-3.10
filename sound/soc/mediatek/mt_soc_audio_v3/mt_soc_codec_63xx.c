...
#define GPIO_EXT_SPKAMP_EN_PIN    (GPIO59 |0x80000000)
#define EXT_SPKAMP_EN_PIN   (GPIO59 |0x80000000)
#define EXT_SPKAMP_MUTE_PIN   (GPIO121 |0x80000000)

static void Ext_Speaker_Amp_Change(bool enable)  // use this--
{
#define SPK_WARM_UP_TIME      (200) // (30) //unit is ms //25
#ifndef CONFIG_FPGA_EARLY_PORTING
    int ret;
    ...
    if (enable)
    {
      ...
      //  mt_set_gpio_out(pin_extspkamp, GPIO_OUT_ZERO); // low disable
       mt_set_gpio_mode(EXT_SPKAMP_EN_PIN, GPIO_MODE_00); //GPIO117: DPI_D3, mode 0
        mt_set_gpio_pull_enable(EXT_SPKAMP_EN_PIN, GPIO_PULL_ENABLE);
        mt_set_gpio_dir(EXT_SPKAMP_EN_PIN, GPIO_DIR_OUT); // output
        mt_set_gpio_out(EXT_SPKAMP_EN_PIN, GPIO_OUT_ONE); // low disable

	 mt_set_gpio_mode(EXT_SPKAMP_MUTE_PIN, GPIO_MODE_00); //GPIO117: DPI_D3, mode 0
        mt_set_gpio_pull_enable(EXT_SPKAMP_MUTE_PIN, GPIO_PULL_ENABLE);
        mt_set_gpio_dir(EXT_SPKAMP_MUTE_PIN, GPIO_DIR_OUT); // output
        
        //mt_set_gpio_out(EXT_SPKAMP_MUTE_PIN, GPIO_OUT_ONE); // low disable
      ...
      //mt_set_gpio_out(EXT_SPKAMP_EN_PIN, GPIO_OUT_ONE); // high enable
      
        mt_set_gpio_out(EXT_SPKAMP_MUTE_PIN, GPIO_OUT_ONE); // high enable
    ...
    else
    {
      ...
      //  mt_set_gpio_out(pin_extspkamp, GPIO_OUT_ZERO); // low disbale
		...
		mt_set_gpio_dir(EXT_SPKAMP_EN_PIN, GPIO_DIR_OUT); // output
		mt_set_gpio_out(EXT_SPKAMP_EN_PIN, GPIO_OUT_ZERO); // low disbale

	mt_set_gpio_dir(EXT_SPKAMP_MUTE_PIN, GPIO_DIR_OUT); // output
	mt_set_gpio_out(EXT_SPKAMP_MUTE_PIN, GPIO_OUT_ZERO); // low disbale
...
{
#define SPK_WARM_UP_TIME        (25) //unit is ms //25
#ifndef CONFIG_FPGA_EARLY_PORTING
...
//            Ana_Set_Reg(AUDENC_ANA_CON6, 0x1515, 0xffff); //default value
        ...
        if (ADCType == AUDIO_ANALOG_DEVICE_IN_ADC1) //main and headset mic
        {
            ...
            if (mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] == 0) //"ADC1", main_mic
            {
                ...
                {
//                    Ana_Set_Reg(AUDENC_ANA_CON9, 0x0217, 0xff1f); //Enable MICBIAS0 and MICBIAS1, MISBIAS0 = 1P9V, [K2] ALPS01824949
                    Ana_Set_Reg(AUDENC_ANA_CON9, 0x0717, 0xff1f); //Enable MICBIAS0 and MICBIAS1, MISBIAS0 = 2P5V, [K2] ALPS01824949
                    
                }
                else if (ECMmode == 2) //single end
                {
//                    Ana_Set_Reg(AUDENC_ANA_CON9, 0x0213, 0xff1f); //Enable MICBIAS0 and MICBIAS1, MISBIAS0 = 1P9V, [K2] ALPS01824949
                    Ana_Set_Reg(AUDENC_ANA_CON9, 0x0713, 0xff1f); //Enable MICBIAS0 and MICBIAS1, MISBIAS0 = 2P5V
                    
                }                
                else //MEMS
                {                    
//                    Ana_Set_Reg(AUDENC_ANA_CON9, 0x0211, 0xff1f); //Enable MICBIAS0 and MICBIAS1, MISBIAS0 = 1P9V , [K2] ALPS01824949         
...
//                Ana_Set_Reg(AUDENC_ANA_CON15, 0x0002, 0x000f); //Audio L PGA 12 dB gain
            ...
            else if (mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] == 1) //"ADC2", headset mic
            {
                ...
                {
                    Ana_Set_Reg(AUDENC_ANA_CON9, 0x0730, 0xfff0); //Enable MICBIAS1 and MICBIAS1, MISBIAS0 = 1P9V, [K2] ALPS01824949
                }                
                else //MEMS
                {
